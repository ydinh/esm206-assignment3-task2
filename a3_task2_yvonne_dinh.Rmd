...
#the third name in row #49 gets dropped because R only recognized the first dash
```
...
mpg_model_class <- mpg %>% # Start with mpg
  unite(col = model_class, # Create a new column called 'model_class'
        c(model, class), # That combines the model & class column
        sep = "-",# separate by dash
...
some_dates_converted <- mdy(some_dates) # Converts to Date, month day year
some_dates_converted #check it out!
...
gasol_hookshots=lakers %>% 
  filter(player=="Pau Gasol") %>% #keep pau gasol data
  filter(type=="hook") %>% #keep hookshot data 
  mutate(new_date=ymd(date)) %>%  #update date column
  mutate(new_month=month(new_date)) #create a new column that holds dates

#Find the *number* of Gasol's hook shots recorded (not necessarily scored) by **month** (hint: `count(month)`) for the 2008 - 2009 season
#i used the length function, count wasn't working for me....
hookshots_by_month=gasol_hookshots %>% 
  group_by(new_month) %>% 
  summarize(total_shots=length(new_month))
```
...
storms_danger <- storms %>% # Start from storms data
  mutate(danger = case_when( # Add a new column season, which contains... 
    status=="hurricane" ~ "high",
    TRUE ~ "moderate" #set all other statuses to equal 'moderate' danger
  )
  )
